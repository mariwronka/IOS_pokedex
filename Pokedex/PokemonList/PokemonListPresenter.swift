...
class PokemonListPresenter: NSObject {
    ...
    private let idsKeys = "favorite.ids"
    
    override init() {
        super.init()
        
        if let data = UserDefaults.standard.array(forKey: idsKeys) as? [Int] {
    ...
    private var favoritesIds = Set<Int>() {
        didSet {
            ...
            UserDefaults.standard.set(Array(favoritesIds), forKey: idsKeys )
        }
    }
...
extension PokemonListPresenter {
    ...
    func fetchData() { self.interactor.fetchData() }
